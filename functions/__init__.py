...
def renderFrames(frameRange, projectName):
    ...
    if scn.nameOutputFiles != "":
        nameOutputFilesString = " -O " + scn.nameOutputFiles
    else:
        nameOutputFilesString = ""

    # runs blender command to render given range from the remote server
    renderCommand = "ssh " + bpy.props.hostServerLogin + " 'python " + scn.tempFilePath + "blender_task -p -vv -n " + projectName + nameOutputFilesString + " -l " + frameRange + " --hosts_file " + scn.tempFilePath + "servers.txt -R " + scn.tempFilePath + " --local_sync " + scn.tempFilePath + projectName + "/toRemote/'"
